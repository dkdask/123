...
import { detectGenreFromTrack, SpotifyTrackWithGenres } from '@/lib/genre-detection';
...
export default function LikesPage() {
  ...
  const handleSearchResultClick = (result: SearchResult) => {
    // Add detected genres to selected
        ...
        >
          <div className="relative">
            <input
              ...
              className="w-full px-6 py-4 bg-white border border-gray-300 rounded-full text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#C5D93D] text-center"
            />
            {isSearching ? (
              <motion.div
                animate={{ rotate: 360 }}
                  ...
                  >
                    ...
                    <div className="flex gap-1">
                      {result.genres.map((genre) => (
                        <span
                          key={genre}
                          className="px-2 py-1 text-xs bg-[#E8F5A3] text-gray-700 rounded-full"
                        >
            ...
            {selectedGenres.map((genreId) => {
              ...
              return (
                <motion.span
                  ...
                  initial={{ scale: 0 }}
                  animate={{ scale: 1 }}
                  className="px-3 py-1 bg-[#C5D93D] text-black rounded-full text-sm font-medium flex items-center gap-1"
                >
                  {genre?.name || genreId}
                  <button
                    onClick={() => handleGenreSelect(genreId)}
                    className="ml-1 w-4 h-4 flex items-center justify-center hover:bg-black/10 rounded-full"
                  >
                    ×
                  </button>
                </motion.span>
